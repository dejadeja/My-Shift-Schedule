...
-(void)markSelected:(BOOL)blnSelected
{
    ...
    if([self.date isDateToday]){
        self.dateLabelContainer.backgroundColor = (blnSelected)?[UIColor whiteColor]: MEDIUM_GREY;
        
        self.dateLabel.textColor = (blnSelected)? LIGHT_ORANGE  :[UIColor whiteColor];
    }else{
        self.dateLabelContainer.backgroundColor = (blnSelected)?[UIColor whiteColor]: [UIColor clearColor];
        
        self.dateLabel.textColor = (blnSelected)?ORANGE:[self colorByDate];
    }
}
-(UIColor *)colorByDate
{
    return [self.date isPastDate]?[UIColor whiteColor]:[UIColor whiteColor];
}
